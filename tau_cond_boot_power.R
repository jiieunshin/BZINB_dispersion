...
cond_tau_null <- function(num) {
  ...
  logL_H0 <- function(param) {
    ...
    v1 <- log(exp(z %*% gamma) + exp(-mu1) * (1 + tau2 * mu2)^{-1/tau2} * (1 + w * (1 - c1) * (1 - c2)))[ind]
    ...
    logbp <- sum(v1) + sum(v2) + sum(v3) - sum(log(1+exp(z %*% gamma)))
  ...
  boot_cric <- matrix(0, nrow = iter, ncol = 2)
  
  for (i in 1:iter) {
    set.seed(i)
    sample_data <- RBNBZI(n = param_grid[num, 3], .2, .4, .4, .8, -1.2, 2.4, 
                          w = param_grid[num, 1], tau1 = param_grid[num, 2], tau2 = .8)
    
    one <- try(optim(par = rep(0, 8), lower = c(rep(-Inf, 5), -5, -5, 1e-10), fn = logL_H0, method = "L")$par, silent = TRUE)
    ...
    for(b in 1:B) {
      ...
      oneB <- try(optim(par = rep(0, 8), lower = c(rep(-Inf, 5), -5, -5, 1e-10), fn = logL_H0B, method = "L")$par, silent = TRUE)
      ...
      B_ctic <- quantile(boot_b, c(.95, .9), na.rm = TRUE)
...
con_boot_power <- parLapply(cl, X = 1:36, cond_tau_null)
result_list


save(con_boot_power, file = "~/tau_con_boot_power.Rdata")
...
# 스코어 결과
for(ii in 1:36){
  cat("n =", param_grid[ii, 3], "tau2 =", param_grid[ii, 1], "w =", param_grid[ii, 2], '\n')
  cat(round(colMeans(con_boot_power[[ii]][[2]]), 3), '\n')
}
