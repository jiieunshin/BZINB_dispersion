...
please <- function(num) {
  ...
  for(i in 1:iteration) {
    set.seed(i)
    sample_data <- RBNBZI(n = param_grid[num, 3], b10 = .2, b11 = .4, b20 = .4, b21 = .8, gam10 = -1.2, gam11 = 2.4, w = param_grid[num, 1], tau1 = param_grid[num, 2], tau2 = 0.8 )
...
tau_power <- parLapply(cl, X = 1:36, please)
tau_power
stopCluster(cl)
...
for(ii in 1:36){
  ...
  for(qq in 1:2){
    cat("true q = ", qval[qq], "estimated prob =", round(mean(tau_power[[ii]]>= kupp[qq], na.rm = T), 3), '\n')
  }
}


save(tau_power, file = "~/tau_cond_power.RData")
